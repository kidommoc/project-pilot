...
```
workspace/
├── current-spec.md ─────→ ../docs/specs/YYYY-MM-DD-{name}.md
...
Design Agent 写 spec → docs/specs/
    ↓ 创建 symlink workspace/current-spec.md
...
L0 检查 open/ → 还有？→ 重复；没有？→ CI/CD
    ↓
CI/CD Agent：审计、发布、删除 workspace/current-spec.md + workspace/meta.md → 回到 Idle
```
...
| Design Agent (L1) | docs/roadmap.md, docs/architecture.md | docs/specs/, docs/architecture.md, workspace/current-spec.md (symlink) |
| Plan Agent (L1) | docs/specs/, workspace/current-spec.md | docs/contracts/meta-*.md, docs/contracts/{feature,fix}/, workspace/meta.md (symlink), workspace/contracts/open/ (symlinks) |
...
| CI/CD Agent (L1) | 全部, workspace/meta.md | docs/roadmap.md (勾选), CHANGELOG.md, 删 workspace/current-spec.md + workspace/meta.md |
...
| 2 | 无 `workspace/current-spec.md` | Idle | 等待 Design |
| 3 | 有 current-spec，无 `workspace/meta.md` | Plan (meta) | Spawn plan |
...
| 7 | current-spec 在，open/ 和 in_progress/ 都空 | Done | Spawn cicd |

CI/CD 完成后删 `workspace/current-spec.md` + `workspace/meta.md` → 回到 #2 Idle。
