...
1. Execute test verification (Quality Gate)
...
- Draft Contracts → `contracts/draft/`
- Open Contracts → `contracts/open/`
- In-Progress Contract → `contracts/in_progress/` (exactly 1)
- Archived Contracts → `contracts/archived/`
- Interface docs → `references/interfaces/`
...
2. Check contracts/in_progress/
   ├─ 1 file → Continue implementation
   ├─ 0 files → Check contracts/open/ → Start next or ask
   └─ >1 files → ERROR: Report to human
```
